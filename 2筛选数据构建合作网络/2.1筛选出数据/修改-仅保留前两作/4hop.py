...
with open(path_dblp, 'r', encoding='utf-8') as f:
    for row in f:
        ...
        for author in authors:
            ...
            if id_i in three_hop_collaborator_ids:
                flag = True
                pass
            pass
        if flag is True:
            ...
            for id in paper_i_ids:
                if cnt >= 2:
                    break
                four_hop_collaborator_ids.add(id)
                cnt += 1
            pass
        pass
    pass

with open(path_fourHop_collaborator, 'w', encoding='utf-8') as f:
    f.write("fourHopID\n")
    for id in four_hop_collaborator_ids:
        if id not in three_hop_collaborator_ids_FULL and\
