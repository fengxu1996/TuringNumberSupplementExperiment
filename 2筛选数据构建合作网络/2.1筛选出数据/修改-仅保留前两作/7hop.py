...
six_hop_collaborator_ids_FULL = set()  # 6 hop id FULL

seven_hop_collaborator_ids = set()  # 7 hop id
...
path_threeHop_collaborator = r"E:\XF\图灵数补充实验\处理数据集\修改-仅保留前几作\3hop_collabor_IDs.txt"
path_fourHop_collaborator = r"E:\XF\图灵数补充实验\处理数据集\修改-仅保留前几作\4hop_collabor_IDs.txt"
path_fiveHop_collaborator = r"E:\XF\图灵数补充实验\处理数据集\修改-仅保留前几作\5hop_collabor_IDs.txt"
path_sixHop_collaborator = r"E:\XF\图灵数补充实验\处理数据集\修改-仅保留前几作\6hop_collabor_IDs.txt"
path_sevenHop_collaborator = r"E:\XF\图灵数补充实验\处理数据集\修改-仅保留前几作\7hop_collabor_IDs.txt"
...
path_sixHop_collaborator_FULL = r"E:\XF\图灵数补充实验\处理数据集\6hop_collabor_IDs.txt"
...
with open(path_sixHop_collaborator, 'r', encoding='utf-8') as f:
    ...
    for row in f:
        if flag is False:
            ...
            continue
        row = row.rstrip('\n')
        six_hop_collaborator_ids.add(int(row))
    pass
...
with open(path_fiveHop_collaborator_FULL, 'r', encoding='utf-8') as f:
    ...
    pass
# read six hop id FULL
with open(path_sixHop_collaborator_FULL, 'r', encoding='utf-8') as f:
    flag = False
    for row in f:
        if flag is False:
            flag = True
            continue
        row = row.rstrip('\n')
        six_hop_collaborator_ids_FULL.add(int(row))
    pass
...
with open(path_dblp, 'r', encoding='utf-8') as f:
    for row in f:
        ...
        for author in authors:
            ...
            if id_i in six_hop_collaborator_ids:
                flag = True
                pass
            pass
        if flag is True:
            ...
            for id in paper_i_ids:
                if cnt >= 2:
                    break
                seven_hop_collaborator_ids.add(id)
                cnt += 1
            pass
        pass
    pass

with open(path_sevenHop_collaborator, 'w', encoding='utf-8') as f:
    f.write("sevenHopID\n")
    for id in seven_hop_collaborator_ids:
        if id not in six_hop_collaborator_ids_FULL and \
                id not in five_hop_collaborator_ids_FULL and\
