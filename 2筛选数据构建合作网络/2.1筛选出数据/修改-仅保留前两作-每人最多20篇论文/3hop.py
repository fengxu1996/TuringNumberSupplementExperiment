...
twoHopID_Num_dict = defaultdict(lambda: 0)

# path_oneHop_collaborator_ = r"E:\XF\TuringNumberSupplementExperiment\处理数据集\1hop_collabor_IDs.txt"
# path_twoHop_collaborator_ = r"E:\XF\TuringNumberSupplementExperiment\处理数据集\2hop_collabor_IDs.txt"
# path_threeHop_collaborator_ = r"E:\XF\TuringNumberSupplementExperiment\处理数据集\3hop_collabor_IDs.txt"
...
with open(path_dblp, 'r', encoding='utf-8') as f:
    for row in f:
        ...
        for author in authors:
            ...
            if id_i in two_hop_collaborator_ids:
                flag = True
                matchedID = id_i
                if twoHopID_Num_dict[id_i] > 20:
                    paperNumLimit = True
                    break
                pass
            pass
        ...
        if flag is True:
            ...
            for id in paper_i_ids:
                if cnt >= 2:
                    break
                three_hop_collaborator_ids.add(id)
                cnt += 1
                xxx.append(id)
...
with open(path_threeHop_collaborator, 'w', encoding='utf-8') as f:
    f.write("threeHopID\n")
    for id in three_hop_collaborator_ids:
        if id not in two_hop_collaborator_ids_FULL and \
