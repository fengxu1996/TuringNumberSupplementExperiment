...
# path_papersWith7hop = r"E:\XF\TuringNumberSupplementExperiment\处理数据集\papersWithin7hop.txt"
path_collaborEdge_cnt = r"E:\XF\图灵数补充实验\处理数据集\collaborEdge_cnt.txt"

collaborEdge_cnt = defaultdict(lambda: 0)
set_turing_1to6HopIDs = set()
# read turing id
for i in range(1, sheet.nrows):
    row = sheet.row_values(i)
    id = str(row[4]).split(',')[0]
    set_turing_1to6HopIDs.add(int(id))
    pass
# read one hop id
with open(path_oneHop_collaborator, 'r', encoding='utf-8') as f:
    ...
    for row in f:
        if flag is False:
            ...
            continue
        row = row.rstrip('\n')
        set_turing_1to6HopIDs.add(int(row))
    pass
# read two hop id
...
with open(path_dblp, 'r', encoding='utf-8') as f:
    for row in f:
        ...
        for author in authors:
            id_i = author['id']
            id_list.append(id_i)
            if id_i in set_turing_1to6HopIDs:
                flag = True
                pass
            pass
        if flag is True:
            ...
            for i in range(0, len(id_list)-1):
                for j in range(i+1, len(id_list)):
                    pair = sorted([id_list[i], id_list[j]])
                    collaborEdge_cnt[(pair[0], pair[1])] += 1
                    pass
                pass
            pass
        pass
...
with open(path_collaborEdge_cnt, 'w', encoding='utf-8') as fOut:
    for key in collaborEdge_cnt.keys():
        fOut.write(str(key[0])+'\t'+str(key[1])+'\t'+str(collaborEdge_cnt[key])+'\n')
    pass
